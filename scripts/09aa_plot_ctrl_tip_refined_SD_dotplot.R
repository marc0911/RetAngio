...
main <- function() {
  stage_name <- "09aa_plot_ctrl_tip_refined_SD_dotplot"

  input_rds <- "results/objects/09o_rpca_tipcell_ctrl_only_resolution_sweep.rds"
  output_dir <- "plots/09aa_rpca_tipcell_ctrl_only_refined_SD_dotplot"
  ...
  seu$tip_ctrl_cluster_plot <- factor(cluster_values, levels = cluster_levels)
  ...
  marker_panel <- list(
    "S-tip refined" = c("Igfbp3", "Esm1", "Apln", "Angpt2", "Col15a1"),
    "D-tip refined" = c("Spock2", "Apod", "Cldn5", "Slc38a5", "Itm2a")
  )
  ...
  write_csv(
    ...
    file.path(review_dir, "09aa_ctrl_tip_refined_SD_dotplot_genes_checked.csv")
  )
  ...
  write_csv(
    dp_data,
    file.path(review_dir, "09aa_ctrl_tip_refined_SD_dotplot_data.csv")
  )
  ...
  p <- ggplot(dp_data, aes(x = features.plot, y = id)) +
    ...
    labs(
      title = "RPCA control-only tip-cell object - refined S/D marker panel",
      x = "Genes",
      y = "Local cluster"
    ) +
  ...
  CairoPDF(
    file.path(output_dir, "09aa_ctrl_tip_refined_SD_dotplot.pdf"),
    width = 13,
    height = 7
  )
  print(p)
  ...
  CairoPNG(
    file.path(output_dir, "09aa_ctrl_tip_refined_SD_dotplot.png"),
    width = 13,
    height = 7,
    units = "in",
    dpi = 300
  ...
  CairoPDF(
    file.path(output_dir, "09aa_ctrl_tip_refined_SD_dotplot_NOlegend.pdf"),
    width = 13,
    height = 7
  )
  print(p_nolegend)
  ...
  CairoPNG(
    file.path(output_dir, "09aa_ctrl_tip_refined_SD_dotplot_NOlegend.png"),
    width = 13,
    height = 7,
    units = "in",
    dpi = 300
