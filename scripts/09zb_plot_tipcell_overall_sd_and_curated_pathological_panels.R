...
get_cluster_levels <- function(cluster_values) {
...
standard_dotplot <- function(
  ...
  log_message("Genes present for ", output_prefix, ": ", paste(genes_present, collapse = ", "))
...
main <- function() {
  stage_name <- "09zb_plot_tipcell_overall_sd_and_curated_pathological_panels"

  input_rds <- "results/objects/09k_rpca_tipcell_resolution_sweep.rds"
  output_dir <- "plots/09zb_rpca_tipcell_overall_sd_and_curated_pathological_panels"
  ...
  seu$tip_overall_cluster_plot_09zb <- factor(cluster_values, levels = cluster_levels)
  Idents(seu) <- "tip_overall_cluster_plot_09zb"
  ...
  standard_dotplot(
    ...
    output_prefix = "09zb_overall_tip_SD_marker_panel",
  ...
  standard_dotplot(
    seu = seu,
    marker_groups = extended_marker_groups,
    cluster_levels = cluster_levels,
    assay_to_use = assay_to_use,
    output_prefix = "09zb_overall_tip_SD_plus_curated_pathological_marker_panel",
    output_dir = output_dir,
    review_dir = review_dir,
    width = 20,
    height = 8
