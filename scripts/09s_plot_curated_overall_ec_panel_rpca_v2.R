...
main <- function() {
  stage_name <- "09s_plot_curated_overall_ec_panel_rpca_v2"

  input_rds <- "results/objects/09h_ec_after_soupx_rpca_annotated.rds"
  output_dir <- "plots/09s_rpca_overall_ec_curated_panel_v2"
  ...
  ## RPCA cluster -> annotation mapping
  rpca_label_map <- c(
  ...
  readr::write_csv(
    ...
    file.path(review_dir, "09s_rpca_curated_panel_v2_genes_checked.csv")
  )
  ...
  CairoPDF(file.path(output_dir, "09s_rpca_overall_ec_curated_dotplot_v2.pdf"), width = 20, height = 10)
  print(p)
  dev.off()

  CairoPNG(
    file.path(output_dir, "09s_rpca_overall_ec_curated_dotplot_v2.png"),
    width = 20,
    height = 10,
    units = "in",
    dpi = 300
