...
main <- function() {
  stage_name <- "09t_plot_ctrl_tip_curated_dotplot"

  input_rds <- "results/objects/09o_rpca_tipcell_ctrl_only_resolution_sweep.rds"
  output_dir <- "plots/09t_rpca_tipcell_ctrl_only_curated_dotplot"
  ...
  marker_panel <- list(
    ...
    "optional extras" = c("Cxcr4", "Dll4", "Mfsd2a")
  )
  ...
  readr::write_csv(
    ...
    file.path(review_dir, "09t_ctrl_tip_curated_dotplot_genes_checked.csv")
  )
  ...
  CairoPDF(file.path(output_dir, "09t_rpca_ctrl_tip_curated_dotplot.pdf"), width = 15, height = 8)
  print(p)
  dev.off()

  CairoPNG(
    file.path(output_dir, "09t_rpca_ctrl_tip_curated_dotplot.png"),
    width = 15, height = 8, units = "in", dpi = 300
  )
  print(p)
  dev.off()

  CairoPDF(file.path(output_dir, "09t_rpca_ctrl_tip_curated_dotplot_NOlegend.pdf"), width = 15, height = 8)
  print(p_nolegend)
  dev.off()

  CairoPNG(
    file.path(output_dir, "09t_rpca_ctrl_tip_curated_dotplot_NOlegend.png"),
    width = 15, height = 8, units = "in", dpi = 300
  )
  print(p_nolegend)
  dev.off()
