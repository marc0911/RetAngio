...
main <- function() {
  ...
  outdir <- "plots/10x_presentation_tipcells_overall"
  out_csv <- "results/abundance/10x_tipcells_overall_cluster_relative_abundance_per_sample.csv"
  cluster_col <- "tip_res_0_20"

  dir.create(outdir, recursive = TRUE, showWarnings = FALSE)
  dir.create("results/abundance", recursive = TRUE, showWarnings = FALSE)

  message_ts("Starting stage 10x_presentation_plots_tipcells_overall")
  ...
  p_sample <- retangio_plot_umap_discrete(
    ...
    xlim = lims$xlim,
    ylim = lims$ylim,
    title_text = "RPCA tip-cell subset — colored by sample",
    legend_title = "Sample"
  )

  retangio_save_plot(
    plot_obj = p_sample + theme(legend.position = "none"),
    filename_base = file.path(outdir, "10x_tipcells_overall_umap_by_sample_NOlegend"),
    width = 10,
    height = 8
  )
  ...
  retangio_save_plot(
    ...
    filename_base = file.path(outdir, "10x_tipcells_overall_umap_by_sample_legendOnly"),
    width = 4,
    height = 5
  )
  ...
  p_abund <- retangio_plot_abundance(
    df = abundance_df,
    title_text = "Overall tip-cell local clusters — relative abundance by sample",
    legend_title = "Tip cluster"
  )

  retangio_save_plot(
    plot_obj = p_abund + theme(legend.position = "none"),
    filename_base = file.path(outdir, "10x_tipcells_overall_cluster_relative_abundance_per_sample_NOlegend"),
    width = 10,
    height = 8
  )
  ...
  retangio_save_plot(
    ...
    filename_base = file.path(outdir, "10x_tipcells_overall_cluster_relative_abundance_per_sample_legendOnly"),
    width = 4,
    height = 6
  )

  message_ts("Saved abundance table:", out_csv)
  message_ts("Stage 10x_presentation_plots_tipcells_overall complete")
}
