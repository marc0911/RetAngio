...
main <- function() {
  stage_name <- "09v_plot_overall_tip_curated_dotplot_v2"

  input_rds <- "results/objects/09k_rpca_tipcell_resolution_sweep.rds"
  output_dir <- "plots/09v_rpca_tipcell_overall_curated_dotplot_v2"
  ...
  seu$tip_cluster_plot <- as.character(seu@meta.data[[cluster_col]])

  cluster_levels <- sort(unique(seu$tip_cluster_plot))
  ...
  seu$tip_cluster_plot <- factor(seu$tip_cluster_plot, levels = cluster_levels)
  Idents(seu) <- "tip_cluster_plot"
  ...
  readr::write_csv(
    ...
    file.path(review_dir, "09v_overall_tip_curated_dotplot_v2_genes_checked.csv")
  )
  ...
  p <- ggplot(dp_data, aes(x = features.plot, y = id)) +
    ...
    labs(
      title = "RPCA overall tip-cell object - curated marker panel",
      x = "Genes",
      y = "Local cluster"
    ) +
  ...
  CairoPDF(file.path(output_dir, "09v_rpca_overall_tip_curated_dotplot_v2.pdf"), width = 11, height = 8)
  print(p)
  dev.off()

  CairoPNG(
    file.path(output_dir, "09v_rpca_overall_tip_curated_dotplot_v2.png"),
    width = 11, height = 8, units = "in", dpi = 300
  )
  print(p)
  dev.off()

  CairoPDF(file.path(output_dir, "09v_rpca_overall_tip_curated_dotplot_v2_NOlegend.pdf"), width = 11, height = 8)
  print(p_nolegend)
  dev.off()

  CairoPNG(
    file.path(output_dir, "09v_rpca_overall_tip_curated_dotplot_v2_NOlegend.png"),
    width = 11, height = 8, units = "in", dpi = 300
  )
  print(p_nolegend)
  dev.off()
