...
main <- function() {
  input_rds <- "results/objects/09a_ec_after_soupx_rpca_integrated.rds"
  output_rds <- "results/objects/09b_ec_after_soupx_rpca_resolution_sweep.rds"
  outdir <- "plots/09b_ec_after_soupx_rpca_resolution_sweep"

  dir.create("results/objects", recursive = TRUE, showWarnings = FALSE)
  dir.create(outdir, recursive = TRUE, showWarnings = FALSE)

  message_ts("Starting stage 09b_resolution_sweep_ec_after_soupx_rpca")
  ...
  if (!"integrated.rpca" %in% names(seu@reductions)) {
    stop("integrated.rpca reduction not found in object.")
  }

  resolutions <- seq(0.1, 1.0, by = 0.1)

  message_ts("Running FindNeighbors on integrated.rpca")
  seu <- FindNeighbors(
    object = seu,
  ...
  message_ts("Running UMAP on integrated.rpca")
  seu <- RunUMAP(
    object = seu,
    reduction = "integrated.rpca",
    dims = 1:30,
  ...
  lims <- retangio_get_fixed_umap_limits(seu, "umap.rpca")
  ...
  if ("plot_condition" %in% colnames(seu@meta.data) && any(!is.na(seu$plot_condition))) {
    ...
    p_cond <- retangio_plot_umap_discrete(
      ...
      xlim = lims$xlim,
      ylim = lims$ylim,
      title_text = "UMAP — RPCA — colored by condition",
    ...
    retangio_save_plot_pdf_png_safe(
      ...
      filename_base = file.path(outdir, "09b_rpca_UMAP_byCondition_NOlegend"),
    ...
    retangio_save_plot_pdf_png_safe(
      plot_obj = p_cond_leg,
      filename_base = file.path(outdir, "09b_rpca_UMAP_byCondition_legendOnly"),
      width = 4,
      height = 3
    )
    ...
    for (cond in retangio_condition_levels) {
      p_hi <- retangio_plot_umap_highlight(
        ...
        xlim = lims$xlim,
        ylim = lims$ylim,
        title_text = paste0("UMAP — RPCA — highlight ", retangio_condition_labels[[cond]]),
        legend_title = "Condition"
      ) + theme(legend.position = "none")

      retangio_save_plot_pdf_png_safe(
        plot_obj = p_hi,
        filename_base = file.path(outdir, paste0("09b_rpca_UMAP_highlight_", cond)),
        width = retangio_width_main,
        height = retangio_height_main
      )
    }
  ...
  for (res in resolutions) {
    ...
    p_cluster <- retangio_plot_umap_discrete(
      ...
      xlim = lims$xlim,
      ylim = lims$ylim,
      title_text = paste0("UMAP — RPCA — colored by cluster — res ", sprintf("%.1f", res)),
      legend_title = "Cluster",
      legend_ncol = 2
    )

    stem <- file.path(outdir, paste0("09b_rpca_res_", sprintf("%.1f", res), "_UMAP_byCluster"))
  ...
  p_tree <- clustree::clustree(
    ...
    labs(title = "Clustree — RPCA EC after SoupX")

  retangio_save_plot_pdf_png_safe(
    plot_obj = p_tree,
    filename_base = file.path(outdir, "09b_rpca_clustree"),
    width = 12,
    height = 10
  )

  message_ts("Stage 09b_resolution_sweep_ec_after_soupx_rpca complete")
}
