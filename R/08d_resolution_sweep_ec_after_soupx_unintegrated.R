...
main <- function() {
  ...
  output_rds <- "results/objects/08d_ec_after_soupx_unintegrated_resolution_sweep.rds"
  ...
  message_ts("Starting stage 08d_resolution_sweep_ec_after_soupx_unintegrated")

  dec <- yaml::read_yaml(decision_file)
  dims_use <- as.integer(unlist(dec$selected_dims))
  if (length(dims_use) == 0) stop("No dims found in decision file.")
  ...
  message_ts("Running FindNeighbors once")
  seu <- FindNeighbors(
    object = seu,
    reduction = "pca",
    dims = dims_use,
  ...
  lims <- retangio_get_fixed_umap_limits(seu, "umap.unintegrated")
  ...
  if ("plot_condition" %in% colnames(seu@meta.data) && any(!is.na(seu$plot_condition))) {
    ...
    p_cond <- retangio_plot_umap_discrete(
      ...
      xlim = lims$xlim,
      ylim = lims$ylim,
      title_text = "UMAP — colored by condition",
    ...
    retangio_save_plot_pdf_png_safe(
      ...
      filename_base = file.path(outdir, "08d_unintegrated_UMAP_byCondition_NOlegend"),
    ...
    retangio_save_plot_pdf_png_safe(
      plot_obj = p_cond_leg,
      filename_base = file.path(outdir, "08d_unintegrated_UMAP_byCondition_legendOnly"),
      width = 4,
      height = 3
    )
    ...
    for (cond in retangio_condition_levels) {
      p_hi <- retangio_plot_umap_highlight(
        ...
        xlim = lims$xlim,
        ylim = lims$ylim,
        title_text = paste0("UMAP — highlight ", retangio_condition_labels[[cond]]),
        legend_title = "Condition"
      ) + theme(legend.position = "none")

      retangio_save_plot_pdf_png_safe(
        plot_obj = p_hi,
        filename_base = file.path(outdir, paste0("08d_unintegrated_UMAP_highlight_", cond)),
        width = retangio_width_main,
        height = retangio_height_main
      )
    }
  ...
  for (res in resolutions) {
    ...
    p_cluster <- retangio_plot_umap_discrete(
      ...
      xlim = lims$xlim,
      ylim = lims$ylim,
      title_text = paste0("UMAP — colored by cluster — res ", sprintf("%.1f", res)),
      legend_title = "Cluster",
      legend_ncol = 2
    )

    stem <- file.path(outdir, paste0("08d_unintegrated_res_", sprintf("%.1f", res), "_UMAP_byCluster"))
  ...
  p_tree <- clustree::clustree(
    ...
    labs(title = "Clustree — unintegrated EC after SoupX")

  retangio_save_plot_pdf_png_safe(
    plot_obj = p_tree,
    filename_base = file.path(outdir, "08d_unintegrated_clustree"),
    width = 12,
    height = 10
  )

  message_ts("Stage 08d_resolution_sweep_ec_after_soupx_unintegrated complete")
}
