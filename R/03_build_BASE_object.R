...
main <- function() {
  ...
  seu <- load_object(file.path(outdir, "02_clean_reintegrated"), cfg)
  ...
  out_obj <- save_object(seu, file.path(outdir, "03_BASE_object"), cfg, object_type = "seurat")

  meta <- tibble::tibble(
    object = basename(out_obj),
    object_size_bytes = lobstr::obj_size(seu),
    cells = ncol(seu),
    genes = nrow(seu)
  )
  write_csv(meta, file.path(outdir, "03_BASE_object_summary.csv"))
  write_checksum(out_obj, file.path(outdir, "03_BASE_object_checksum.csv"))
