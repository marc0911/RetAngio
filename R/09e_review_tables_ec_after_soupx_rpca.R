...
main <- function() {
  ...
  input_markers <- "results/markers/09d_ec_after_soupx_rpca_findallmarkers.csv"

  out_cluster_sizes <- "results/review/09e_ec_after_soupx_rpca_cluster_sizes.csv"
  out_top20 <- "results/review/09e_ec_after_soupx_rpca_top20_markers.csv"
  out_panel <- "results/review/09e_ec_after_soupx_rpca_marker_panel_avgexp.csv"

  dir.create("results/review", recursive = TRUE, showWarnings = FALSE)

  message_ts("Starting stage 09e_review_tables_ec_after_soupx_rpca")
  ...
  message_ts("Stage 09e_review_tables_ec_after_soupx_rpca complete")
}
