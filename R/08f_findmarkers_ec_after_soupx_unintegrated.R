...
main <- function() {
  ...
  output_markers <- "results/markers/08f_ec_after_soupx_unintegrated_findallmarkers.csv"
  output_top10 <- "results/markers/08f_ec_after_soupx_unintegrated_top10_markers.csv"

  dir.create("results/markers", recursive = TRUE, showWarnings = FALSE)

  message_ts("Starting stage 08f_findmarkers_ec_after_soupx_unintegrated")
  ...
  message_ts("Stage 08f_findmarkers_ec_after_soupx_unintegrated complete")
}
