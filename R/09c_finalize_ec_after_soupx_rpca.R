...
main <- function() {
  decision_file <- "decisions/09c_ec_after_soupx_rpca_resolution_choice.yaml"
  output_rds <- "results/objects/09c_ec_after_soupx_rpca_final.rds"

  dir.create("results/objects", recursive = TRUE, showWarnings = FALSE)

  message_ts("Starting stage 09c_finalize_ec_after_soupx_rpca")
  ...
  seu@misc$stage09c_rpca_finalize <- list(
  ...
  message_ts("Saved final RPCA object:", output_rds)
  ...
  message_ts("Stage 09c_finalize_ec_after_soupx_rpca complete")
}
