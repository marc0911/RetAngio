...
main <- function() {
  ...
  input_markers <- "results/markers/08f_ec_after_soupx_unintegrated_findallmarkers.csv"

  out_cluster_sizes <- "results/review/08g_ec_after_soupx_unintegrated_cluster_sizes.csv"
  out_top20 <- "results/review/08g_ec_after_soupx_unintegrated_top20_markers.csv"
  out_panel <- "results/review/08g_ec_after_soupx_unintegrated_marker_panel_avgexp.csv"

  dir.create("results/review", recursive = TRUE, showWarnings = FALSE)

  message_ts("Starting stage 08g_review_tables_ec_after_soupx_unintegrated")
  ...
  message_ts("Stage 08g_review_tables_ec_after_soupx_unintegrated complete")
}
