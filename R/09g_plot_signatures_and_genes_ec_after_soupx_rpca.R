...
main <- function() {
  ...
  input_sig_used <- "results/signatures/09f_ec_after_soupx_rpca_signatures_used.csv"
  input_cluster_scores <- "results/signatures/09f_ec_after_soupx_rpca_cluster_mean_scores.csv"
  input_xlsx <- "reference/RetAngio_annotation_master_v2.xlsx"

  outdir <- "plots/09g_ec_after_soupx_rpca_signatures_and_genes"

  dir.create(outdir, recursive = TRUE, showWarnings = FALSE)

  message_ts("Starting stage 09g_plot_signatures_and_genes_ec_after_soupx_rpca")
  ...
  reduction_to_use <- if ("umap.rpca" %in% Reductions(seu)) {
    "umap.rpca"
  } else if ("umap" %in% Reductions(seu)) {
    "umap"
  } else {
    stop("No RPCA UMAP reduction found in object.")
  }
  ...
  for (i in seq_len(nrow(sig_plot_tbl))) {
    ...
    retangio_save_plot(
      plot_obj = p_vln,
      filename_base = file.path(outdir, paste0("09g_signature_violin_", cat_safe)),
      width = 7,
      height = 6
    )
  }
  ...
  for (i in seq_len(nrow(sig_plot_tbl))) {
    ...
    retangio_save_plot(
      plot_obj = p_feat,
      filename_base = file.path(outdir, paste0("09g_signature_feature_", cat_safe)),
      width = 7,
      height = 6
    )
  }
  ...
  retangio_save_plot(
    plot_obj = p_heat,
    filename_base = file.path(outdir, "09g_signature_cluster_mean_heatmap"),
    width = 10,
    height = 6
  )
  ...
  for (g in main_genes) {
    ...
    retangio_save_plot(
      plot_obj = p_gene_vln,
      filename_base = file.path(outdir, paste0("09g_gene_violin_", g)),
      width = 7,
      height = 6
    )
  }
  ...
  for (g in main_genes) {
    ...
    retangio_save_plot(
      ...
      filename_base = file.path(outdir, paste0("09g_gene_feature_", g)),
      width = 7,
      height = 6
    )
  }
  ...
  retangio_save_plot(
    plot_obj = p_dot,
    filename_base = file.path(outdir, "09g_gene_dotplot_main_genes"),
    width = 16,
    height = 8
  )

  message_ts("Stage 09g_plot_signatures_and_genes_ec_after_soupx_rpca complete")
}
