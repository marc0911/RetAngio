...
main <- function() {
  ...
  out_rds <- "results/objects/09l_rpca_tipcell_zarkada_scored.rds"
  out_sig <- "results/signatures/09l_rpca_tipcell_zarkada_signatures_used.csv"
  out_cell <- "results/signatures/09l_rpca_tipcell_zarkada_cell_scores.csv"
  out_cluster_mean <- "results/signatures/09l_rpca_tipcell_zarkada_cluster_mean_scores.csv"
  out_cluster_sizes <- "results/review/09l_rpca_tipcell_cluster_sizes_res_0_20.csv"
  plot_dir <- "plots/09l_rpca_tipcell_zarkada"
  ...
  message_ts("Starting stage 09l_score_zarkada_tip_programs_rpca")
  ...
  if (!"umap.tip" %in% Reductions(seu)) {
    stop("umap.tip reduction not found.")
  }

  seu <- retangio_add_plotting_metadata(seu)

  # ------------------------------------------------------------------
  # Zarkada-inspired exploratory signatures
  # ------------------------------------------------------------------
  # Keep this explicit and transparent for this stage.
  # We can refine after reviewing the output.
  s_tip_genes_raw <- c(
  ...
  d_tip_genes_raw <- c(
    ...
    "Efnb2", "Igf2", "Igf1r", "Vegfr2", "Vegfr3"
  )

  row_genes <- rownames(seu)

  map_present <- function(genes, universe) {
    hit <- genes[genes %in% universe]
    unique(hit)
  }
  ...
  if (length(s_tip_genes) < 3) {
    stop("Too few S-tip genes found in object.")
  }
  if (length(d_tip_genes) < 3) {
    stop("Too few D-tip genes found in object.")
  }
  ...
  # Seurat names the new metadata columns with trailing 1
  s_col <- "zarkada_S_tip1"
  d_col <- "zarkada_D_tip1"

  if (!all(c(s_col, d_col) %in% colnames(seu@meta.data))) {
    stop("Module score columns were not created as expected.")
  }

  seu$tip_cluster_0_20 <- factor(
    as.character(seu@meta.data[[cluster_col]]),
    levels = sort(unique(as.character(seu@meta.data[[cluster_col]])),
                  na.last = TRUE)
  )
  ...
  cell_df <- seu@meta.data %>%
    ...
    transmute(
      ...
      sample_id = if ("sample_id" %in% colnames(.)) as.character(sample_id) else as.character(orig.ident),
      plot_condition = as.character(plot_condition_fixed),
      tip_cluster_0_20 = as.character(tip_cluster_0_20),
  ...
  # Plots
  # ------------------------------------------------------------------
  lims <- retangio_get_fixed_umap_limits(seu, "umap.tip")

  cluster_levels <- levels(seu$tip_cluster_0_20)
  cluster_pal <- retangio_make_cluster_palette(cluster_levels)

  df_cluster <- retangio_make_umap_df(
    obj = seu,
    reduction = "umap.tip",
    color_col = "tip_cluster_0_20",
  ...
  p_cluster <- retangio_plot_umap_discrete(
    ...
    xlim = lims$xlim,
    ylim = lims$ylim,
    title_text = "RPCA tip-cell subset — local clusters (res 0.20)",
  ...
  save_plot_safe(
    ...
    file.path(plot_dir, "09l_tipcluster_res_0_20_NOlegend"),
  ...
  save_plot_safe(
    p_cluster_leg,
    file.path(plot_dir, "09l_tipcluster_res_0_20_legendOnly"),
    width = 4.5,
  ...
  p_s <- FeaturePlot(
    ...
    ggtitle("Zarkada S-tip score") +
    retangio_theme_proj()

  save_plot_safe(
    p_s,
    file.path(plot_dir, "09l_zarkada_S_tip_score_featureplot"),
    width = retangio_width_main,
    height = retangio_height_main
  )
  ...
  p_d <- FeaturePlot(
    ...
    ggtitle("Zarkada D-tip score") +
    retangio_theme_proj()

  save_plot_safe(
    p_d,
    file.path(plot_dir, "09l_zarkada_D_tip_score_featureplot"),
    width = retangio_width_main,
    height = retangio_height_main
  )
  ...
  p_s_vln <- VlnPlot(
    object = seu,
    features = s_col,
    group.by = "tip_cluster_0_20",
    pt.size = 0,
    assay = "RNA"
  ) +
    ggtitle("Zarkada S-tip score by local tip cluster") +
    xlab("tip_res_0_20") +
    ylab("Module score") +
    retangio_theme_proj()
  ...
  save_plot_safe(
    p_s_vln,
    file.path(plot_dir, "09l_zarkada_S_tip_score_violin_by_cluster"),
    width = 9,
    height = 6
  )
  ...
  p_d_vln <- VlnPlot(
    ...
    ggtitle("Zarkada D-tip score by local tip cluster") +
    xlab("tip_res_0_20") +
    ylab("Module score") +
    retangio_theme_proj()
  ...
  save_plot_safe(
    p_d_vln,
    file.path(plot_dir, "09l_zarkada_D_tip_score_violin_by_cluster"),
    width = 9,
    height = 6
  )
  ...
  p_dot <- DotPlot(
    ...
    RotatedAxis() +
    ggtitle("Zarkada S-tip / D-tip genes by local tip cluster") +
    retangio_theme_proj()

  save_plot_safe(
    p_dot,
    file.path(plot_dir, "09l_zarkada_tip_gene_dotplot_by_cluster"),
    width = 12,
    height = 7
  )

  message_ts("Stage 09l_score_zarkada_tip_programs_rpca complete")
}
